...
#pragma pack(pop)
	...
	BloomFilter(size_t expectedElemNum, double fpr, unsigned hashNum,
			unsigned kmerSize) :
			m_filter(NULL), m_size(0), m_hashNum(hashNum), m_kmerSize(kmerSize), m_dFPR(fpr), m_nEntry(
					0), m_tEntry(0) {
		if (m_hashNum == 0) {
		...
		if (m_size == 0) {
		    ...
		    // fprintf(stderr, "optimal m_size: %lu\n", m_size);
			/*size_t ret = 1;
		    while (ret < m_size) {
		        ret = ret << 1;
		    }
		    m_size = ret;
		    fprintf(stderr, "m_size: %lu\n", m_size);*/
		}
		initSize(m_size);
	}
	...
	/*void insert(vector<size_t> const &precomputed) {
		...
		for (size_t i = 0; i < m_hashNum; ++i) {
			...
			// __sync_or_and_fetch(&m_filter[normalizedValue / bitsPerChar], bitMask[normalizedValue % bitsPerChar]);
			m_filter[normalizedValue >> 3] |= bitMask[normalizedValue & bitsPerChar];
		}
	}*/
	 ...
	 * Accepts a list of precomputed hash values. Faster than rehashing each time.
	 */
	inline void insert(const size_t precomputed[]) {

		//iterates through hashed values adding it to the filter
		for (size_t i = 0; i < m_hashNum; ++i) {
			size_t normalizedValue = precomputed[i] % m_size;
			// size_t normalizedValue = precomputed[i] & (m_size - 1);
			// fprintf(stderr, "%llu %llu\n", normalizedValue, precomputed[i] % m_size);
			// exit(0);
			// __sync_or_and_fetch(&m_filter[normalizedValue / bitsPerChar],
			// 	bitMask[normalizedValue % bitsPerChar]);
			__sync_or_and_fetch(&m_filter[normalizedValue >> 3], bitMask[normalizedValue & 7]);
			// m_filter[normalizedValue >> 3] |= bitMask[normalizedValue & 7];
		}
	}
	 ...
	 */
	inline bool insertAndCheck(const size_t precomputed[]) {
		//iterates through hashed values adding it to the filter
		bool found = true;
		for (size_t i = 0; i < m_hashNum; ++i) {
			size_t normalizedValue = precomputed[i] % m_size;
	 ...
	 * Returns if already inserted
	 */
	inline bool insertAndCheck(vector<size_t> const &precomputed) {
		//iterates through hashed values adding it to the filter
		bool found = true;
		for (size_t i = 0; i < m_hashNum; ++i) {
			size_t normalizedValue = precomputed.at(i) % m_size;
	 ...
	 * Accepts a list of precomputed hash values. Faster than rehashing each time.
	 */
	/*inline bool contains(vector<size_t> const &precomputed) const {
		for (size_t i = 0; i < m_hashNum; ++i) {
			size_t normalizedValue = precomputed.at(i) % m_size;
	 ...
	 * Accepts a list of precomputed hash values. Faster than rehashing each time.
	 */
	inline bool contains(const size_t precomputed[]) const {
		for (size_t i = 0; i < m_hashNum; ++i) {
			size_t normalizedValue = precomputed[i] % m_size;
			// size_t normalizedValue = precomputed[i] & (m_size - 1);
			// unsigned char bit = bitMask[normalizedValue % bitsPerChar];
			unsigned char bit = bitMask[normalizedValue & 7];
			// if ((m_filter[normalizedValue / bitsPerChar] & bit) != bit) {
			if ((m_filter[normalizedValue >> 3] & bit) != bit) {
				return false;
			}
		}
		return true;
	...
	inline friend std::ostream& operator<<(std::ostream& out, const BloomFilter& o)
	{
		assert(out);
		o.writeHeader(out);
		assert(out);
...
protected:
	 ...
	 */
	inline void initSize(size_t size) {
		if (size % 8 != 0) {
		...
		if (m_filter != NULL)
			delete[] m_filter;
		// fprintf(stderr, "m_sizeInBytes: %u\n", m_sizeInBytes);
	 ...
	 */
	inline size_t calcOptimalSize(size_t entries, double fpr) const {
	 ...
	 */
	inline double calcFPR_hashNum(unsigned hashFunctNum) const {
